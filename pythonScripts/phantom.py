...
materials = {'air':{'ID':1, 'density':0.0012, 'color':'white',
                    'filePath':'MCGPUmaterial/air.mcgpu'},
             'water':{'ID':2, 'density':1.0, 'color':'midnightblue',
                      'filePath':'MCGPUmaterial/water.mcgpu'},
             'caffeine':{'ID':3, 'density':1.23, 'color':'brown',
                      'filePath':'MCGPUmaterial/caffeine/caffeineMIF_1.23_narrow_5_150keV.mcgpu'},                      
             '2.0Iodine':{'ID':4, 'density':1.00015943, 'color':'orange',
                          'filePath':'MCGPUmaterial/2.0Iodine_5-200keV.mcgpu'},
             '10.0Iodine':{'ID':5, 'density':1.00398580, 'color':'yellow',
...
class Phantom (object):
    ...
    @property
    def voxelSize (self): return self._voxelSize
    @voxelSize.setter    
    ...
    @property
    def numVoxels (self): return self._numVoxels
    @numVoxels.setter    
    ...
    @property
    def backgroundMaterial (self): return self._backgroundMaterial
    ...
    @property
    def digitalPhantom (self): return np.array (self._digitalPhantom)
    ...
    def _check_voxels (self, infoname, voxelinfo):
        ...
        if (voxelx <= 0 or type(voxelx) != atype):
            raise ValueError("Error: {0} in x-axis must be greater than zero and {1}".format(infoname, atypename))
        elif (voxely <= 0 or type(voxely) != atype):
            raise ValueError("Error: {0} in y-axis must be greater than zero and {1}".format(infoname, atypename))
        elif (voxelz <= 0 or type(voxelz) != atype):
            raise ValueError("Error: {0} in z-axis must be greater than zero and {1}".format(infoname, atypename))

    def _check_material (self, material):
        if not material in materials.keys():
            raise ValueError("Error: Invalid material {0}. Please provide one of the followings: {1}".format(material, materials.keys()))
        ...
        for z in range(0, numVoxelsZ):
            for y in range(0, numVoxelsY):
                for x in range(0, numVoxelsX):
                    ...
                    if(math.sqrt( ((trueX - centerX) ** 2 ) + ((trueY - centerY) ** 2) ) < radius):
        ...
        for z in range(0, numVoxelsZ):
            for y in range(0, numVoxelsY):
                for x in range(0, numVoxelsX):
                    ...
                    if(math.sqrt( ((trueX - centerX) ** 2 ) + ((trueY - centerY) ** 2) + ((trueZ - centerZ) ** 2)) < radius):
                        self._digitalPhantom[z][y][x] = materials[material]['ID']
    ...
    def writeVoxFile(self, filename):
        ...
        numVoxelsX, numVoxelsY, numVoxelsZ = self._numVoxels
        voxelSizeX, voxelSizeY, voxelSizeZ = self._voxelSize
        massDensities = [value['density'] for _, value in dict (sorted(materials.items(), key=lambda x: x[1]['ID'])).items()]

        with open(filename, 'w') as fp:
            
            fp.write("# HEADER section: voxel geometry file generated by example_phantom_generator.py\n")
            fp.write("[SECTION VOXELS HEADER v.2008-04-13]\n")

            # Number of Voxels
            fp.write("{0} {1} {2} No. OF VOXELS IN X,Y,Z\n".format (numVoxelsX, numVoxelsY, numVoxelsZ))

            # Voxel Size
            fp.write("{0} {1} {2} VOXEL SIZE (cm) ALONG X,Y,Z\n".format (voxelSizeX, voxelSizeY, voxelSizeZ))

            fp.write(" 1                  COLUMN NUMBER WHERE MATERIAL ID IS LOCATED\n")
            fp.write(" 2                  COLUMN NUMBER WHERE THE MASS DENSITY IS LOCATED\n")
            fp.write(" 1                  BLANK LINES AT END OF X,Y-CYCLES (1=YES,0=NO)\n")
            fp.write("[END OF VXH SECTION]  # MCGPU-PET voxel format: Material  Density  Activity\n")
    ...
    def plotPhantom (self, filename='phantom.png', voxelSizeFactors=[1, 1, 1], alpha=0.7):

        numVoxelsX, numVoxelsY, numVoxelsZ = self._numVoxels
        voxelSizeX, voxelSizeY, voxelSizeZ = self._voxelSize
        colors = [value['color'] for _, value in dict (sorted(materials.items(), key=lambda x: x[1]['ID'])).items()]
