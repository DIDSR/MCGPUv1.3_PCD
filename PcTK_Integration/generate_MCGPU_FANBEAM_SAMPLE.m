...
home_folder = pwd
MCGPU_output_folder = '../Sample_Fan_Beam/output/PCD/allPhotons'
output_filename = 'mc-gpu_image'
...
read_binary   =  [0, 1] % 1 for the 1st value in arr indicates we read binary 
                       % the second value is required if binary is enabled
                             ...
                             % 5 - multiscatter data
pixel_size    =  0.01; % mm
Nch           =  100   ; %<--- number of pixles x axis, Nch = geo.nDetector(2);
Nrow          =  1  ; %<--- number of pixles y axis, Nrow = geo.nDetector(1); geo.nDetector=[240; 1];	
Nview         =  180  ; %<--- nb of projections
Estart        =  3   ; %<---  Estart in MCGPU
Eend          =  120  ; %<---  Eend in MCGPU
Nbin          =  90  ; %<--- Number of Energy bins
